...
public class ConstVariable {
  ...
  // 表示 等待队列
  public final static Queue<String> waitQueue = new ArrayBlockingQueue<>(10);
  // 表示操作的 thread 信息
  public final static ConcurrentMap<String, ThreadTask> threadTaskMap = new ConcurrentHashMap<>();
  // 表示计算的最大线程数
  public final static int maxThreadCount = Math.max(2, Runtime.getRuntime().availableProcessors() * 2);
  // 表示 使用的线程个数
